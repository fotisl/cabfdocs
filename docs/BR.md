...
When issuing a DV Certificate for a fully-qualified Domain Name with "onion" as the rightmost label, the CA SHALL confirm that, as of the date the Certificate was issued, the Applicant had control over the .onion Domain Name in accordance with Appendix B.
...
This appendix defines permissible verification procedures for DV Certificates referring to onion services. It does not define or modify procedures or requirements for issuance of any other type of Certificate.

A CA MAY issue a DV Certificate including a subject Domain Name with .onion in the rightmost label of the Domain Name provided that issuance complies with the requirements set forth in this Appendix:

1. The Domain Name contains at least two labels, where the right-most label is "onion", and the label immediately preceding the right-most "onion" label is a valid version 3 onion address.  A valid version 3 onion address must consist of exactly 56 characters, each of which is an alphanumeric ASCII character.  This address is encoded using base32 with a lowercase alphabet, which MUST be converted to uppercase before decoding, as foreseen by section 3.4 of RFC 4648.  The address must also specify onion address version 3 and pass a SHA-256 integrity checksum.

	A Python program to illustrate the process of verifying correctness of labels representing version 3 onion addresses follows:
		...
		if sys.version_info < (3, 6):
		    # This is the python-pysha3 or python3-pysha3 package to
		    # add support for SHA3 to versions of Python prior to 3.6.
		    import sha3
		
		base32decode = lambda x: base64.b32decode(x)
		...
		if len(sys.argv) < 2:
		   fail("Usage: {} onionlabel".format(sys.argv[0]))
		
		label = sys.argv[1].upper()
		
		if len(label) != 56:
		    fail("Wrong length (must be 56 ASCII characters)")
		if not all(c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567" for c in label):
		    fail("Invalid base32 character in label")
		
		dec = base32decode(label)
		
		if dec[34:35] != b"\x03":
		    fail("Unknown onion address version (must be 3)")
		
		if dec[32:34] != sha3_256(b'.onion checksum' + dec[0:32] + dec[34:35])[0:2]:
		    fail("Invalid SHA3 checksum")
		
		print("Valid onion label")
...
2. The CA MUST verify the Applicant’s control over the .onion Domain Name using one of the following:
	...
	b. The CA MAY verify the Applicant’s control over the .onion service by using method 3.2.2.4.9, Test Certificate.

	c. The CA MAY verify the Applicant’s control over the .onion service by using method 3.2.2.4.10, TLS Using a Random Number.

	d. The CA MAY verify the Applicant’s control over the .onion service by having the Applicant provide a Certificate Request signed using the .onion public key if the Attributes section of the certificationRequestInfo contains:

	(i) A caSigningNonce attribute that (1) contains a single value with at least 64-bits of entropy and (2) is generated by the CA and (ii) An applicantSigningNonce attribute that (1) contains a single value with at least 64-bits of entropy and (2) is generated by the Applicant.
...
3. All other methods of verifying control of Domain Names MUST NOT be used to verify control of an onion service for purposes of issuance of a DV Certificate.

4. Each Certificate that includes a Domain Name where .onion is in the right-most label of the Domain Name MUST conform to the other provisions of these Requirements in every respect. The CA MAY include a wildcard character in the Subject Alternative Name Extension and Subject Common Name Field as the left-most character in the .onion Domain Name provided inclusion of the wildcard character complies with Section 3.2.2.6 of these Requirements.

5. When a Certificate includes a Domain Name where .onion is in the right-most label of the Domain Name, the Domain Name shall not be considered an Internal Name if the Certificate was issued in compliance with this Appendix B.

6. A single DV Certificate may apply to both subject Domain Names that are .onion names and subject Domain Names that are not .onion names, as long as the issuing CA has validly verified the Applicant’s control of each and every subject name using a method approved for that purpose by these Requirements.
